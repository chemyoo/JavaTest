...
public class Main {
    ...
    public static String myEmailAccount = "chemyoodeveloper@foxmail.com";
    public static String myEmailPassword = "hrxzixgbgqizbhdb";//授权码
    ...
    // 收件人邮箱（替换为自己知道的有效邮箱）
    public static String receiveMailAccount = "chemyoo_15@163.com";
...
//        transport.close();
        ...
        grego.set(GregorianCalendar.SECOND, 0);
        if (grego.before(Calendar.getInstance())) {  
        	grego.add(GregorianCalendar.DAY_OF_MONTH, 1);
        }  
        //每天定时发送
...
//					e.printStackTrace();
//				}
//			}}, grego.getTime(), 1*24*60*60*1000L);
        sendEmail();
    }
	public static void sendEmail() throws Exception {
        ...
        //Authenticator auth = new MailAuthenticator(myEmailAccount,myEmailPassword); // 使用验证，创建一个Authenticator
        Session session = Session.getInstance(props);// 根据Properties，Authenticator创建Session
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail) throws Exception {
        ...
        // 2. From: 发件人
        message.setFrom(new InternetAddress(sendMail, sendMail.substring(0,
        		sendMail.lastIndexOf("@")), "UTF-8"));

        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, receiveMail.substring(0,
        		receiveMail.lastIndexOf("@")), "UTF-8"));
